...
export default function SearchScreen() {
  ...
  return (
    <AppScreen assistant background={colors.paper}>
      ...
      <View style={styles.searchBar}><Feather name="search" size={20} color={colors.secondary} /><TextInput accessibilityLabel="Search your thoughts" accessibilityHint="Searches titles, notes, and categories as you type" value={query} onChangeText={setQuery} autoCapitalize="none" autoCorrect={false} autoFocus placeholder="Search your records…" placeholderTextColor={colors.muted} returnKeyType="search" style={styles.input} />{query.length > 0 ? <Pressable accessibilityLabel="Clear search" accessibilityRole="button" accessibilityHint="Removes the current search" onPress={() => setQuery('')} style={({ pressed }) => [styles.clear, pressed && styles.clearPressed]}><Feather name="x" size={18} color={colors.secondary} /></Pressable> : null}</View>
      {!query ? <View style={styles.ready}><View style={styles.readyIcon}><Feather name="search" size={22} color={colors.muted} /></View><View style={styles.readyMain}><Text style={styles.readyTitle}>Type anything you remember</Text><Text style={styles.help}>LifeDesk searches only your real saved records; it does not show sample or suggested history.</Text></View></View> : <View style={styles.results}>
        <Text accessibilityRole="header" style={styles.resultCount}>{results.length} {results.length === 1 ? 'result' : 'results'} for “{query.trim()}”</Text>
        ...
        {!results.length ? <View style={styles.empty} accessibilityLiveRegion="polite"><View style={styles.emptyIcon}><Feather name="search" size={24} color={colors.muted} /></View><Text style={styles.emptyTitle}>No matching records</Text><Text style={styles.emptyText}>Try another word, a shorter phrase, or a category like task.</Text></View> : null}
      </View>}
    </AppScreen>
  );
}
...
const styles = themedStyles(() => ({
  ...
  empty: { alignItems: 'center', paddingTop: 54, paddingHorizontal: 18 },
  emptyIcon: { width: 52, height: 52, borderRadius: 26, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.background },
  emptyTitle: { marginTop: 14, fontFamily: fonts.bodySemiBold, fontSize: 16, color: colors.ink },
  emptyText: { marginTop: 5, textAlign: 'center', fontFamily: fonts.body, fontSize: 13, lineHeight: 19, color: colors.secondary },
}));
