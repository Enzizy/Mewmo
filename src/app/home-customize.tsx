...
export default function HomeCustomizeScreen() {
  ...
  const [draftReady, setDraftReady] = useState(hydrated);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    if (!hydrated || draftReady) return;
    setDraft(homePreferences);
    setDraftReady(true);
  }, [draftReady, homePreferences, hydrated]);
  ...
  const save = async () => {
    ...
    setSaving(true);
    try { await updateHomePreferences(draft); router.replace('/'); }
  ...
  return <AppScreen>
    <ScreenHeader back />
    <PageHeader title="Customize Home" supporting="Keep important information visible and move supporting widgets out of the way." action={<Pressable accessibilityLabel="Save Home layout" accessibilityRole="button" disabled={!draftReady || saving} onPress={save} style={[styles.save, (!draftReady || saving) && styles.disabled]}><Text style={styles.saveText}>{saving ? 'Saving…' : 'Save'}</Text><Feather name="check" size={16} color={colors.paper} /></Pressable>} />

    {!draftReady ? <View style={styles.loading}><ActivityIndicator color={colors.ink} /><Text style={styles.loadingText}>Loading your Home layout…</Text></View> : <>
